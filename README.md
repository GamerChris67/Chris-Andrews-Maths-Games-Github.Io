...
<h1>🎯 Chris Andrew Maths Game 🎯</h1>

<div class="container">

<div id="menu">
    <h2>Select Mode</h2>
    <button onclick="startIndividual()">👤 Individual Mode</button>
    <button onclick="startTeam()">👥 Team Mode (24 Students)</button>
</div>
...
function startIndividual() {
    ...
    document.getElementById("modeInfo").innerText = "👤 Individual Mode";
    nextQuestion();
}
...
function startTeam() {
    ...
    document.getElementById("menu").classList.add("hidden");
    document.getElementById("game").classList.remove("hidden");
    document.getElementById("modeInfo").innerText = "👥 Team 1 of 4 (6 students)";
    nextQuestion();
}
...
function nextQuestion() {
    ...
    questionCount++;
    timer = 15;
    document.getElementById("timer").innerText = "⏱️ 15";
    ...
    interval = setInterval(() => {
        ...
        if (timer <= 0) {
            ...
            streak = 0;
            sounds.time.play();
            showMessage("Never give up – Chris 💪", false);
            nextQuestion();
        }
    }, 1000);
}
...
function answer(index) {
    ...
    if (chosen === currentAnswer) {
        ...
        sounds.correct.play();
        sounds.win.play();
        showMessage("Nice one – Chris 🌟", true);
    } else {
        streak = 0;
        sounds.wrong.play();
        showMessage("Never give up – Chris 💪", false);
